...
const handleFeeItemChange = (e) => {
  ...
  // Store which input is currently focused before state update
  const activeElement = document.activeElement;
  const isFeeTypeInput = activeElement === feeTypeRef.current;
  const isAmountInput = activeElement === amountRef.current;
  
  setFeeItem(prev => ({ ...prev, [name]: value }));
  
  // Restore focus after state update
  requestAnimationFrame(() => {
    if (isFeeTypeInput && feeTypeRef.current) {
      feeTypeRef.current.focus();
    } else if (isAmountInput && amountRef.current) {
...
const addFeeItem = () => {
  ...
  // Focus back to fee type input for next entry
  setTimeout(() => {
