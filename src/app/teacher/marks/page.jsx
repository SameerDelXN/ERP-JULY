...
export default function MarksPage() {
 ...
 const saveMarksToDatabase = async () => {
  try {
    ...
    const marksData = {
      ...
      students: allStudents.map((student) => ({
        studentId: student._id || student.id, // Ensure we're using the correct ID field
    ...
    // Update local state with the new marks
    setAllStudents(prevStudents => 
      prevStudents.map(student => {
        const updatedStudent = result.data?.years
          ?.find(y => y.year === marks.year)?.semesters
          ?.find(s => s.semester === marks.semester)?.divisions
          ?.find(d => d.name === marks.division)?.subjects
          ?.find(sub => sub.name === marks.subject)?.exams
          ?.find(e => e._id === marks.examId)?.result
          ?.find(r => r.student.toString() === student._id || r.student.toString() === student.id);
  ...
  const fetchStudents = async (division) => {
    try {
      setLoading(true);
      const url = `/api/teachers/${user.id}/students?division=${division}${
  ...
  const fetchTeacherSubjects = async () => {
    try {
      ...
      const examsData = data.myExam?.[0]?.exams || [];
  ...
  const handleSemChange = (e) => {
    ...
    const semesterData = teacherData.filter(
      (item) => item.year === marks.year && item.semester === selectedSemester
    );
  ...
  const handleDivisionChange = async (e) => {
    ...
    const divisionData = teacherData.find(
      (item) =>
        item.year === marks.year &&
        item.semester === marks.semester &&
                ...
                {currentStudents.length > 0 ? (
                  currentStudents.map((student) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="w-16 bg-indigo-100 rounded-full h-2.5 mr-2">
                            <div
                              className="bg-indigo-600 h-2.5 rounded-full"
                              style={{ width: `${student.marks || 0}%` }}
                            ></div>
                          </div>
                  ...
                  <tr>
                    <td
                      colSpan="6"
                      className="px-6 py-4 text-center text-indigo-500"
                    >
