import { connectToDatabase } from '@/app/lib/mongodb';
import studentSchema from '@/app/models/studentSchema';
import { NextResponse } from 'next/server';


export async function GET(request) {
  try {
    await connectToDatabase();

    // Get query parameters
    const { searchParams } = new URL(request.url);
    const page = parseInt(searchParams.get('page')) || 1;
    const limit = parseInt(searchParams.get('limit')) || 10;
    const search = searchParams.get('search');
    const sortField = searchParams.get('sort') || 'createdAt';
    const sortOrder = searchParams.get('order') === 'desc' ? -1 : 1;

    // Build query
    const query = {};
    
    // Add search filter if provided
    if (search) {
      query.$or = [
        { name: { $regex: search, $options: 'i' } },
        { email: { $regex: search, $options: 'i' } },
        { studentId: { $regex: search, $options: 'i' } }
      ];
    }

    // Get total count for pagination
    const total = await studentSchema.countDocuments(query);

    // Fetch students with pagination and sorting
    const students = await studentSchema.find(query, { admissionId: 0 })
      .sort({ [sortField]: sortOrder })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();

    return NextResponse.json({
      success: true,
      data: students,
      pagination: {
        page,
        limit,
        total,
        totalPages: Math.ceil(total / limit)
      }
    }, { status: 200 });

  } catch (error) {
    console.error('[GET_ALL_STUDENTS_ERROR]', error);
    return NextResponse.json(
      { success: false, error: 'Internal Server Error' },
      { status: 500 }
    );
  }
}

// import Student from '@/app/models/studentSchema';
// import Academic from '@/app/models/academicSchema';
// import { connectToDatabase } from '@/app/lib/mongodb';
// import { NextResponse } from 'next/server';

// export async function GET(request) {
//   try {
//     await connectToDatabase();

//     // Get query parameters
//     const { searchParams } = new URL(request.url);
//     const page = parseInt(searchParams.get('page')) || 1;
//     const limit = parseInt(searchParams.get('limit')) || 10;
//     const search = searchParams.get('search');
//     const sortField = searchParams.get('sort') || 'createdAt';
//     const sortOrder = searchParams.get('order') === 'desc' ? -1 : 1;

//     // Build query
//     const query = {};
    
//     // Add search filter if provided
//     if (search) {
//       query.$or = [
//         { fullName: { $regex: search, $options: 'i' } },
//         { email: { $regex: search, $options: 'i' } },
//         { studentId: { $regex: search, $options: 'i' } }
//       ];
//     }

//     // Get all students with pagination
//     const students = await Student.find(query)
//       .sort({ [sortField]: sortOrder })
//       .skip((page - 1) * limit)
//       .limit(limit)
//       .lean();

//     // Get all academic records
//     const academics = await Academic.find({}).lean();
    
//     // Map students with their academic information
//     const studentsWithAcademics = students.map(student => {
//       let academicInfo = {
//         department: null,
//         year: null,
//         division: null
//       };
      
//       // Find the academic record that contains this student
//       for (const academic of academics) {
//         for (const year of academic.years) {
//           for (const division of year.divisions) {
//             if (division.students.some(s => s.toString() === student._id.toString())) {
//               academicInfo = {
//                 department: academic.department,
//                 year: year.year,
//                 semester: year.semester,
//                 division: division.name,
//                 programType: academic.programType
//               };
//               break;
//             }
//           }
//           if (academicInfo.department) break;
//         }
//         if (academicInfo.department) break;
//       }
      
//       return {
//         ...student,
//         academicInfo
//       };
//     });

//     // Get total count for pagination
//     const total = await Student.countDocuments(query);

//     return NextResponse.json({
//       success: true,
//       data: studentsWithAcademics,
//       pagination: {
//         page,
//         limit,
//         total,
//         totalPages: Math.ceil(total / limit)
//       }
//     });

//   } catch (error) {
//     console.error('[GET_STUDENTS_WITH_ACADEMICS_ERROR]', error);
//     return NextResponse.json(
//       { success: false, error: error.message },
//       { status: 500 }
//     );
//   }
// }