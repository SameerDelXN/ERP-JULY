...
export async function POST(request) {
  ...
  try {
    ...
    const existingBranchesResult = await academicSchema.find({}, 'department');
    const existingBranches = new Set(existingBranchesResult.map(doc => doc.department.trim()));
    ...
    for (const [index, row] of jsonData.entries()) {
      ...
      // Validate branch against existing departments in database
      if (admissionData.branch && !existingBranches.has(admissionData.branch.trim())) {
        hasError = true;
        rowErrors.push(`Invalid branch: "${admissionData.branch}" does not exist in the database`);
      }
      ...
      return NextResponse.json(
        {
          ...
          validationErrors: validationErrors.map((err) => ({
            row: err.row,
            errors: err.errors,
            data: err.data, // Include the row data in the response
          })),
        },
        { status: 400 }
      );
