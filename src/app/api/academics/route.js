...
export async function POST(req) {
  try {
    ...
    // ✅ Validate department, year, and divisions
    if (!data.department || !data.year || !Array.isArray(data.divisions)) {
      return NextResponse.json({ error: 'Missing department, year, or divisions' }, { status: 400 });
    }

    // Normalize department name
    data.department = data.department.trim();

    // Loop through divisions and validate each subject's teacher
    for (const division of data.divisions) {
      if (!division.name || !Array.isArray(division.subjects)) {
        return NextResponse.json({ error: 'Each division must have a name and subjects' }, { status: 400 });
      }
