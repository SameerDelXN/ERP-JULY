...
//   const now = new Date();
//   // Create exam date at start of day to avoid timezone issues
...
//   if (days < 0) return { status: "past", color: "gray", bgColor: "bg-gray-100" };
//   if (days === 0) return { status: "urgent", color: "red", bgColor: "bg-red-50" };
...
//                 const isExamAvailable = examIsToday && examStatus.status !== "past"
...
//                       {examStatus.status === "urgent" && examStatus.status !== "past" && (
...
function getExamStatus(exam, studentId) {
  ...
  console.log("exammmmmmmmmmm",exam);
  ...
  if (days < 0 || (days === 0 && studentResult)) { 
    ...
    // If no result found, check if student should have attended
    return { status: "missed", color: "red", bgColor: "bg-red-50" };
  }
