...
const Layout = ({ children }) => {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    return staffSidebarItems.filter(
  ...
  useEffect(() => {
    if (!loading && filteredItems.length > 0) {
      ...
      if (!isAllowed) {
        ...
        router.push(firstAllowed === "overview" ? "/staff" : `/staff/${firstAllowed}`);
      }
    }
                  ...
                  <div className="px-4 py-4 border-b border-gray-100/50">
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-gray-900">{user?.username || "Staff User"}</p>
                        <p className="text-sm text-gray-500">{user?.email || "staff@company.com"}</p>
