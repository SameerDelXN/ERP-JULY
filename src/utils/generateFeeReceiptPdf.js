...
export async function generateFeeReceiptPDF(receiptData) {
  try {
    console.log('Generating PDF for receipt:', receiptData);
    ...
    // Debug fee structure specifically
    if (formattedData.feeStructure) {
      console.log('FeeStructure found:', formattedData.feeStructure);
      console.log('FeesFromStudent:', formattedData.feeStructure.feesFromStudent);
      console.log('FeesFromSocialWelfare:', formattedData.feeStructure.feesFromSocialWelfare);
    } else {
      console.log('No feeStructure found in formatted data');
    }
    
    // Read HTML template
    const templatePath = path.join(process.cwd(), 'src', 'templates', 'feeReceipt.html');
    ...
    if (formattedDataWithStatus.feeStructure && formattedDataWithStatus.componentPayments) {
      ...
      html = html.replace('<!-- FEE_ROWS_PLACEHOLDER -->', feeRows);
      
      // Replace total with actual amount paid
      html = html.replace('<!-- TOTAL_FEES_PLACEHOLDER -->', formattedDataWithStatus.amountPaid);
      
      // Show welfare summary if welfare fees exist
      if (hasWelfareFees) {
        html = html.replace('display: none;', 'display: block;');
        html = html.replace('id="welfare-total">0', `id="welfare-total">${welfareTotal}`);
      }
      ...
      // Replace total with total fees
      html = html.replace('<!-- TOTAL_FEES_PLACEHOLDER -->', formattedDataWithStatus.feeStructure.totalFees);
      ...
      console.log('feeStructure exists:', !!formattedData.feeStructure);
      console.log('feesFromStudent exists:', !!(formattedData.feeStructure && formattedData.feeStructure.feesFromStudent));
      console.log('feesFromStudent length:', formattedData.feeStructure ? formattedData.feeStructure.feesFromStudent?.length : 'N/A');
      
      // Fallback
      html = html.replace('<!-- FEE_ROWS_PLACEHOLDER -->', `<tr>
        <td>1</td>
        <td>Fee Payment</td>
        <td class="amount">${formattedData.amountPaid}</td>
      </tr>`);
      html = html.replace('<!-- TOTAL_FEES_PLACEHOLDER -->', formattedData.amountPaid);
    }
